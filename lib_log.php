<?php // lib.php :: Les fontions essentielles au script modifié pour leftnav_log.php.
...
function prettydatenews($uglydate) { //  Format de la date (DD-MM-YYYY pour les news de l'accueil).
...
function display($content, $title, $topnav=true, $leftnav=true, $rightnav=true, $badstart=false) { // Page en fonction du navigateur.
    ...
    $template = gettemplate("primary_log");
    
    if ($rightnav == true) { $rightnav = gettemplate("rightnav"); } else { $rightnav = ""; }
    if ($leftnav == true) { $leftnav = gettemplate("leftnav_log"); } else { $leftnav = ""; }
    if ($topnav == true) {
        $topnav = "";
    } else {
        $topnav = "";
    ...
    $finalarray = array(
        ...
        "leftnav_log"=>parsetemplate($leftnav,$userrow),
        "topnav"=>$topnav,
        "totaltime"=>round(getmicrotime() - $starttime, 4),
        "numqueries"=>$numqueries,
        "version"=>$version,
