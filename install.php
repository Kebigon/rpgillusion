<?php // install.php création des tables
...
<span class="alerte">Note:</span> Veuillez vous assurer que les infos dans config.php, ont été complétées correctement avant de continuer.  L\'installation échouera si ces infos ne sont pas correctes.  En outre, la base de données de MySQL doit exister déjà.  Ce script d\'installation prendra soin d\'installer la structure et le contenu du jeu, mais la base de données elle-même doit déjà exister sur votre serveur MySQL avant d\'éxécuter l\'installation.<br><br>
L\'installation de RPG illusion est un processus en deux étapes simple:  installez les tables de la base de données, puis créez l\'utilisateur d\'administration.  Après ces deux étapes le jeu sera totalement installé.<br><br></div>
...
";
if (dobatch($query) == 1) { echo 'La table Babble Box a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Babble Box.<br>'; }
unset($query); 
...
";
if (dobatch($query) == 1) { echo 'La table Control a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Control.<br>'; }
unset($query);

$query = "
INSERT INTO `$control` VALUES (1, 'RPG illusion v1.3', 250, 1, '', '', 1, '', 'Mage', 'Guerrier', 'Paladin', 'Facile', '1', 'Moyen', '1.2', 'Dur', '1.5', 1, 1, 1, 1, 1, '© RPGillusion.net - Tous droits réservés - 2005-2007. Images protégés par les droits d\'auteurs.');
";
if (dobatch($query) == 1) { echo 'La table Control a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Control a été complétée.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Menu_users a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Menu_users.<br>'; }
unset($query);
$query = "
INSERT INTO `$menu_users` VALUES (1, 'Menu à éditer depuis l\'administration');
";
if (dobatch($query) == 1) { echo 'La table Menu_users a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Menu_users a été complétée.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Drops a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Drops.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Drops a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Drops table a été complétée.<br>'; }
unset($query);
}
...
";
if (dobatch($query) == 1) { echo 'La table History a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table History.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Items a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Items.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Items a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Items a été complétée.<br>'; }
unset($query);
}
...
";
if (dobatch($query) == 1) { echo 'La table Levels a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Levels.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Levels a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Levels a été complétée.<br>'; }
unset($query);
}
...
";
if (dobatch($query) == 1) { echo 'La table Monsters a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Monsters.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Monsters a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Monsters a été complétée.<br>'; }
unset($query);
}
...
if (dobatch($query) == 1) { echo 'La table News a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table News.<br>'; }
unset($query);   

$query = "
INSERT INTO `$news` VALUES (1, '2008-01-01', 'Bienvenue sur rpg illusion','Ceci est la première news de rpg illusion, utilisez l\'administration pour ajouter une news','RPG illusion est un jeu entièrement gratuit en php. Il est téléchargeable sur le site officiel http://www.rpgillusion.net.');
";
if (dobatch($query) == 1) { echo 'La table News a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table News a été complétée.<br>'; }
unset($query);
...
`description` VARCHAR(200) NOT NULL default '',
 ...
 if (dobatch($query) == 1) { echo 'La table Partners a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Partners.<br>'; }
unset($query); 

$query = "
INSERT INTO `$partners` VALUES (1, 'RPG illusion', 'http://www.rpgillusion.net','','Le site officiel de RPG illusion');
";
if (dobatch($query) == 1) { echo 'La table Partners a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Partners a été complétée.<br>'; }
unset($query);
...
`resultat4` int(6) NOT NULL default '0',
 ...
 if (dobatch($query) == 1) { echo 'La table Poll a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Poll.<br>'; }
unset($query);

if ($full == true) {
$query = "
INSERT INTO `$poll` VALUES (1, 'Vous trouvez le jeu','Très bien', 'Bien', 'Bof', 'Nul', '0', '0', '0', '0');
";
if (dobatch($query) == 1) { echo 'La table Poll a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Poll a été complétée.<br>'; }
unset($query);
}  
...
`ip` VARCHAR(100) NOT NULL default ''
 ...
 if (dobatch($query) == 1) { echo 'La table Poll_ip a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Poll_ip.<br>'; }
unset($query);   
...
";
if (dobatch($query) == 1) { echo 'La table Spells a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Spells.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Spells a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Spells a été complétée.<br>'; }
unset($query);
}
...
";
if (dobatch($query) == 1) { echo 'La table Towns a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Towns.<br>'; }
unset($query);
...
";
if (dobatch($query) == 1) { echo 'La table Towns a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Towns a été complétée.<br>'; }
unset($query);
}
...
";
if (dobatch($query) == 1) { echo 'La table Users a été crée.<br>'; } else { echo '<span class="alerte">Erreur</span> de création de la table Users.<br>'; }
unset($query);
...
INSERT INTO `$users` VALUES(3, 'Sarah', '27f98a7d253b3bb9ec4703ff4e61bd90', 'anonyme3@email.com', '1', 'Sarah', '2006-07-30 18:58:09', '2006-12-17 01:18:29', 0, 0, 0, 1, 5, 1, 'En ville', 0, 0, 0, 0, 0, 0, 0, 43, 26, 19, 43, 26, 19, 10, 0, 3078, 210, 20, 21, 15, 31, 239, 19, 17, 29, 31, 31, 1, 'Arc', 'Cobra de Gobi', 'Chaussure de randonnée', 'Memory Drop', 'Memory Drop', 'Life Pebble', 0, '0,1', '0,1,2,3,4');
...
INSERT INTO `$users` VALUES(6, 'Michelle', 'ad0234829205C9033196ba818f7a872b', 'anonyme6@email.com', '1', 'Michelle', '2005-12-09 13:21:23', '2007-10-31 12:36:17', 0, 0, 0, 1, 4, 1, 'En ville', 1, 0, 0, 0, 0, 0, 0, 15, 15, 15, 15, 15, 215, 1, 1, 0, 0, 0, 0, 0, 19, 81, 2, 28, 29, 0, 0, 0, 'Bilboquet', 'Botte de Stella', 'Chaussure de randonnée', 'Aucun', 'Aucun', 'Aucun', 0, '0', '0');
...
";
if (dobatch($query) == 1) { echo 'La table Users a été complétée.<br>'; } else { echo '<span class="alerte">Erreur</span> lorsque la table Users a été complétée.<br>'; }
unset($query);
}
    ...
    echo '<br>Tables Mysql crées en  <b>'.$time.'</b> secondes.<br><br><a href="install.php?page=3">Cliquez ici pour poursuivre l\'installation.</a></div></body></html>';
...
Maintenant vous devez créer un compte d\'administrateur ainsi vous pourrez employer le menu d\'administration du jeu.  Complétez les champs ci-dessous pour créer votre compte.  Vous pourrez modifier vos infos par la suite dans le menu d\'administration.<br><br></div>
...
<tr valign="top"><td style="width:110px">Difficultée:</td><td><select name="difficulty"><option value="1">Facile</option><option value="2">Moyen</option><option value="3">Dur</option></select><br><br></td></tr>
...
<body>
<div><br><span class="mauve1"><b>Installation de RPG illusion: Etape 2</b></span><br><br>
Votre compte d\'administrateur a été crée avec succès. l\'installation est terminée.<br><br>
...
function fifth() { // Appelle de l'auteur de jeu.
    ...
    if (mail("ffstory1@hotmail.com", "", "$url") != true) { die('L\'ajout de votre URL a échouée, vous pouvez recommencer ou terminer l\'installation <a href="index.php">du jeu</a>.'); }
...
Vous desormais <a href="index.php">jouer au jeu</a>. Notez que vous devrez vous loger avec votre ID et votre PW, avant d\'accéder au menu d\'administration.</div>
</body>
</html>';
